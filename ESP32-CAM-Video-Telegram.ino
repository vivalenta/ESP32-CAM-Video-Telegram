...
WiFiManagerParameter dev, id, pass, timzon, hdcam_check;
String chat_id;
String BOTtoken;
String timezone;
int MagicNumber = 11;
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/start") {
      ...
      welcome += "/start: Start\n\n";
      welcome += "/portal: Start WiFi Portal ᯤ\n";
...
bool init_wifi() {
  ...
  new (&dev) WiFiManagerParameter("DevName", "Name of Device", devname, 12);
  new (&id) WiFiManagerParameter("chat_id", "Telegram ID", def_chat_id, 15);
  new (&pass) WiFiManagerParameter("BOTtoken", "Telegram Pass", def_BOTtoken, 50);
  new (&timzon) WiFiManagerParameter("timzon", "Time Zone", def_timezone, 50);
  new (&hdcam_check) WiFiManagerParameter("phdcam", "<p>HD? (or VGA)", def_hdcam, 3, "type=\"checkbox\"");
  ...
  wm.setConnectTimeout(120);       // how long to try to connect for before continuing
...
void loop() {
  ...
  if (portal_request && (!portalRunning)) {
    wm.setConfigPortalTimeout(120);
    portalRunning = true;
    Serial.println("WiFi: Start Config Portal");
